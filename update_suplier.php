...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ubah"])) {
    ...
    $suplierController->updateSuplier($idsuplier, $namasuplier, $alamatsuplier, '1');
    header("Location: list_suplier.php");
    exit();
}

$dataSuplier = $suplierController->ambilSuplierById($idsuplier);
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <input type="hidden" name="idsuplier" value="<?php echo $dataSuplier['idsuplier']; ?>">
            <div class="form-row">
                <div class="col-md-6">
                    <label for="namasuplier">Nama Suplier:</label>
                    <input type="text" class="form-control" id="namasuplier" name="namasuplier" value="<?php echo $dataSuplier['namasuplier']; ?>" required>
                </div>
                <div class="col-md-6">
                    <label for="alamatsuplier">Alamat Suplier:</label>
                    <input type="text" class="form-control" id="alamatsuplier" name="alamatsuplier" value="<?php echo $dataSuplier['alamatsuplier']; ?>" required>
                </div>
            </div>
            <button type="submit" class="btn btn-primary mt-3" name="ubah">Update Suplier</button>
        </form>
