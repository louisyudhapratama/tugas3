...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ubah"])) {
    ...
    $barangController->updateBarang($idbarang, $namabarang, $satuan, $idsuplier, '1');
    header("Location: list_barang.php");
    exit();
}

$dataBarang = $barangController->ambilBarangById($idbarang);
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            <div class="form-row">
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="namabarang" name="namabarang" value="<?php echo $dataBarang['namabarang']; ?>" required>
                </div>
                <div class="col-md-4">
                    <label for="satuan">Satuan:</label>
                    <input type="text" class="form-control" id="satuan" name="satuan" value="<?php echo $dataBarang['satuan']; ?>" required>
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-control" name="idsuplier" id="idsuplier" required>
                        ...
                        while ($row = $suplier->fetch_assoc()) {
                            $selected = ($row['idsuplier'] == $dataBarang['idsuplier']) ? 'selected' : '';
                            echo "<option value='" . $row['idsuplier'] . "' $selected>" . $row['namasuplier'] . "</option>";
                        }
                        ?>
                    </select>
                </div>
            </div>
            <button type="submit" class="btn btn-primary mt-3" name="ubah">Update Barang</button>
        </form>
